...
of such a computation is $[[ SOURCE[error] ]]$. Specifically, because values may only contain one piece of top-level evidence, 
...
is to step to $[[ SOURCE[error] ]]$, which is what happens in \rrule{RedAppEvFail}, \rrule{RedProjFail} and \rrule{RedAscrFail}.
...
\begin{proof}
	...
	\begin{itemize}
		...
		Then $[[e1]]=[[pi @ i (ep (v1 , v2))]]$ and $[[e2]] = [[SOURCE[error] ]]$. 
		Then combining \rrule{TransformProj} with \autoref{lem:canonical-trans} and \autoref{lem:value-expr-trans},
		we have $[[ [|pi @ i (v1 , v2)|]k  ]]$ steps to \\ $[[ (\ x . let y1 := pi1 x in ,,,)[ ( [|ep|], ([|v1|], [|v2|] ))] ]]$.
		If we $\beta$-reduce, and substitute for the let-expressions, we get $[[ PROD @ i [ [|ep|], (\ z1 . ,,,)] ]]$.
		We then apply \autoref{lem:ev-correct} and let-substitution to step to $[[TARGET[error] ]]$. 
		...
		Then $[[e1]]=[[ep1 (ep2 r)]]$ and $[[e2]]=[[SOURCE[error] ]]$.
		Applying \rrule{TransformEv} with \autoref{lem:canonical-trans} and and \autoref{lem:value-expr-trans}, we can see that
		this steps to $[[MEET[ [|ep1|], [|ep2|], (\ y . k[(y, u2) ]) ] ]]$
		where $[[ [|ep2 r|] ]] = [[ ([|ep2|], u2) ]]$. By \autoref{lem:ev-correct} along with our premise, this steps to $[[TARGET[error] ]]$.
