...
we adopt the simpler approach used by \citet{parametricityRevisited}, where evidence ascription
...
First, the AGT approach allows us to take a pre-existing static langauge and introduce gradual types. 
...
Our target language, given in \autoref{fig:target-syntax}, is essentially an untyped version of the $\lambda^K$
 ...
 We have distinct syntactic classes for \textit{values} (metavarible $[[u]]$),
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item \rrule{RedAppEvFail}: then $[[e1]] = [[ ep1 (\ x : T . e') ep2 v ]]$ and $[[e2]]=[[ error ]]$.
		Let $[[( [|ep2|], u)]] = [[ [|v|] ]]$ (by \autoref{lem:canonical-trans}).
        If we apply \autoref{lem:value-expr-trans}, we can see that $[[  [|ep1 (\ x : T . e') ep2 v|]k ]]$
        steps to \\$[[ (\ x1  x2. let y1 := pi1 x1 in ,,,)[( [|ep1|] , (\ x c . [|e'|]c )), ( [|ep2|], u) ]  ]]$.
        We can $\beta$-reduce and apply the let-substitutions to then step to
		...
		and substitute
		to see that $[[ [|(Prod@i ep) vi|]k ]]$ also setps to $[[ MEET[ [|Prod @ i ep|], u1, (\z1' . k [(z1', u2)] )] ]]$.   
